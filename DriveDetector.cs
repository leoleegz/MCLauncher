...
namespace MCLauncher
{
    ...
    class DriveDetector : IDisposable 
    {
        ...
        ManagementOperationObserver managementOperationObserver;

        public void StartCDDetect()
        {
            ...
            eventWatcher = new ManagementEventWatcher(new ManagementScope("root\\CIMV2", opt), eventQuery);

            try
            {
                eventWatcher.EventArrived += new EventArrivedEventHandler(this.CDROM_InstanceModified);
                eventWatcher.Start();
            }
            catch
            { }
        ...
        public void StopCDDetect()
        {
            eventWatcher.Stop();
        }

        public void CDROM_InstanceModified(object sender, EventArrivedEventArgs e)
        {
            PropertyData propertyData = e.NewEvent.Properties["TargetInstance"];

            if (propertyData != null)
            {
                ManagementBaseObject managementBaseObject = propertyData.Value as ManagementBaseObject;

                if (managementBaseObject != null)
                {
                    // if VolumeName == null, volume ejected
                    if (managementBaseObject.Properties["VolumeName"].Value != null)
                    {
                        //foreach (PropertyData data in managementBaseObject.Properties)
                        //{
                        ...
                        {
                            DriveDetectorEventArgs arg = new DriveDetectorEventArgs();
                            arg.Drive = managementBaseObject.Properties["DeviceID"].Value + "\\";
                            tempDeviceArrived(this, arg);
                        }
                    }
                    else
                    {
